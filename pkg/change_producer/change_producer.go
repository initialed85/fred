...
func HandleRepository(ctx context.Context, tx pgx.Tx, repository *api.Repository, repositoriesPath string) error {
	...
	repository.SyncedAt = time.Now().UTC()
...
func Run() error {
	...
	return internal.RunWithTx(
		...
		func(ctx context.Context, db *pgxpool.Pool, tx pgx.Tx) error {
			repository, err := api.ChangeProducerClaimRepository(
				ctx,
				tx,
				...
				fmt.Sprintf(
					...
					api.RepositoryTableSyncedAtColumn,
			...
			defer func() {
				if repository != nil {
					repository.ChangeProducerClaimedUntil = time.Now().UTC()
					err = repository.Update(ctx, tx, false)
					if err != nil {
