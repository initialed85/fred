...
func HandleRepository(ctx context.Context, tx pgx.Tx, repository *api.Repository, repositoriesPath string) error {
	...
	log.Printf("syncing %s", repository.URL)
	...
	for _, branchRef := range branchRefs {
		ref := branchRef.Name().String() // e.g. refs/heads/refactor-1
		parts := strings.Split(ref, "/")
		branch := parts[len(parts)-1] // e.g. refactor-1
		remoteBranch := fmt.Sprintf("remotes/origin/%s", branch)
		remoteRef := fmt.Sprintf("refs/%s", remoteBranch)
	...
	for _, branchRef := range branchRefs {
		...
		log.Printf("checking out %s:%s", repository.URL, ref)

		err = workTree.Checkout(&git.CheckoutOptions{
			Branch: plumbing.ReferenceName(remoteRef),
		})
		if err != nil {
			return fmt.Errorf("checkout for %s:%s failed: %s", repository.URL, ref, err.Error())
		}

		reference, err := gitRepository.Head()
		if err != nil {
			return fmt.Errorf("get head for %s:%s failed: %s", repository.URL, ref, err.Error())
		}

		commitObject, err := gitRepository.CommitObject(reference.Hash())
		if err != nil {
			return fmt.Errorf("get commit for %s:%s failed: %s", repository.URL, ref, err.Error())
		}
		...
		if len(existingChanges) > 0 {
			log.Printf("already know about %s:%s:%s", repository.URL, ref, commitObject.Hash.String())
			continue
		}
	...
	repository.HandledAt = time.Now().UTC()
...
func Run() error {
	...
	return internal.RunWithTx(
		...
		func(ctx context.Context, db *pgxpool.Pool, tx pgx.Tx) error {
			repository, err := api.RepositorySyncerClaimRepository(
				ctx,
				tx,
				time.Now().UTC().Add(claimDuration),
				time.Second*10,
				fmt.Sprintf(
					"now() - %s > interval '%f seconds'",
					api.RepositoryTableHandledAtColumn,
					syncInterval.Seconds(),
				),
			)
			if err != nil {
				return fmt.Errorf("attempt to claim a repository failed: %s", err.Error())
			}

			if repository == nil {
				if !lastRunFoundNothing {
					lastRunFoundNothing = true
					log.Printf("no repositories to claim")
				}
			...
			defer func() {
				if repository != nil {
					repository.RepositorySyncerClaimedUntil = time.Now().UTC()
					err = repository.Update(ctx, tx, false)
					if err != nil {
						err = fmt.Errorf("attempt to release claim on %s failed: %s", repository.URL, err.Error())
			...
			if err != nil {
				return fmt.Errorf("attempt to handle %s failed: %s", repository.URL, err.Error())
			}
